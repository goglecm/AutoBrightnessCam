...
#   define BATTERY_FILE "/sys/class/power_supply/BAT1/status"
...
public:
    void SetUp(void)
    {
        ...
        fake_abc_ioService_setBatteryStatePath(BATTERY_FILE);

        setState(ABC_POWER_CONTROLLER_STATE_FULLY_CHARGED);

        abc_powerController_resetBatteryStatePath();

        ASSERT_TRUE(abc_powerController_setBatteryStatePath(BATTERY_FILE));
...
TEST_F(abc_power_controller_filepaths, battery_state_is_unknown_when_the_battery_state_path_is_not_set)
{
...
TEST_F(abc_power_controller_filepaths, battery_state_is_read)
{
    (void) abc_powerController_isCharging();
